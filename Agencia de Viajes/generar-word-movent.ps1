# Script generar Word MOVENT con diseno de Miku Inn + colores azules

$wordPath = "C:\Proyecto-Final-ADFS-y-DABD1\Agencia de Viajes\Informe-Pruebas-Unitarias-MOVENT.docx"
$mdPath   = "C:\Proyecto-Final-ADFS-y-DABD1\Agencia de Viajes\INFORME-COMPLETO-TESTS-MOVENT.md"
...
# ── COLORES MOVENT (formato BGR de Word COM) ──────────────────────────────────
# Paleta azul/celeste — Agencia de Viajes MOVENT
$wdAuto         = -16777216   # wdColorAutomatic — sin sombreado

$CL_H1          = 0x8C3D1A   # Azul oscuro profundo  (RGB #1A3D8C)
$CL_H2          = 0xB87A3D   # Azul medio            (RGB #3D7AB8)
$CL_H3          = 0xD4A15A   # Azul claro            (RGB #5AA1D4)
$CL_H4          = 0xE9C48B   # Celeste               (RGB #8BC4E9)
$CL_SEPARATOR   = 0xD4A15A   # Igual que H3

$CL_TH_BG       = 0xB87A3D   # Header tabla bg       (RGB #3D7AB8)
$CL_TH_FG       = 0xFFFFFF   # Texto tabla header    (blanco)
$CL_TR_WHITE    = 0xFFFFFF   # Fila blanca
$CL_TR_LBLUE    = 0xFCF4E8   # Fila celeste claro    (RGB #E8F4FC)
$CL_TBL_BORDER  = 0x8C3D1A   # Borde tabla           (RGB #1A3D8C)

$CL_CODE_BG     = 0xF0F5FA   # Fondo bloque codigo   (RGB #FAF5F0 gris azulado)
$CL_CODE_FG     = 0x8C3D1A   # Texto codigo          (RGB #1A3D8C azul oscuro)

Write-Host "Generando Word con diseno Miku Inn + colores MOVENT..." -ForegroundColor Cyan
    ...
    foreach ($tLine in $dataRows) {
        ...
        foreach ($cellVal in $cells) {
            ...
            if ($rowIdx -eq 1) {
                ...
                $bg = if ($rowIdx % 2 -eq 0) { $CL_TR_WHITE } else { $CL_TR_LBLUE }
...
Write-Host " MOVENT - Informe de Tests"                -ForegroundColor Cyan
Write-Host "----------------------------------------" -ForegroundColor DarkCyan
Write-Host '  Diseno  : Miku Inn (estructura + espaciado)'
Write-Host '  Colores : MOVENT (azul/celeste)'
Write-Host '  Tests   : 56 (20 backend + 36 frontend)'
Write-Host '  Estado  : 56/56 CORRECTO (100%)'
Write-Host "----------------------------------------" -ForegroundColor DarkCyan
