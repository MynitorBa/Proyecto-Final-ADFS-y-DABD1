...
// ─── Paleta de colores Movent ─────────────────────────────────────────────────
var C = {
  dark:   [26,  26,  46 ],   // #1a1a2e
  mid:    [22,  33,  62 ],   // #16213e
  blue:   [15,  52,  96 ],   // #0f3460
  accent: [74, 158, 255 ],   // #4a9eff
  white:  [255,255, 255 ],
  text:   [30,  41,  59 ],
  muted:  [100,116, 139 ],
  border: [232,236, 243 ],   // #e8ecf3
  bglt:   [238,246, 255 ],   // #eef6ff
  codebg: [22,  33,  62 ],
  codefg: [226,232, 240 ],
  h2:     [15,  52,  96 ],
  h3:     [74, 158, 255 ]
...
/**
 * Dibuja el header y footer de cada página interior del PDF.
 * Header: banda azul oscura con nombre del sistema y título de la sección.
 * Footer: número de página centrado con línea separadora en el color de marca.
 * @param {object} doc - Instancia de jsPDF.
...
function dibujarHeader(doc, titulo, num) {
  ...
  doc.text('MOVENT DOCS', PL, 9);

  // Título de la sección a la derecha
  tc(doc, [200, 225, 255]);
  doc.setFontSize(7); doc.setFont('helvetica', 'normal');
  var tit = titulo.length > 55 ? titulo.slice(0, 52) + '...' : titulo;
  doc.text(sanitizar(tit), PR, 9, { align: 'right' });

  // Footer con línea y número de página centrado
 ...
 * Reemplaza caracteres fuera de Latin-1 (flechas, comillas tipográficas, em-dashes,
 * bullets, etc.) por equivalentes ASCII visibles para evitar cuadros en el PDF.
...
/**
 * Limpia la sintaxis Markdown de una línea devolviendo texto plano sanitizado.
 * Elimina negritas, cursivas, código inline, links, encabezados, listas y blockquotes.
 ...
 * Maneja h1-h4, bloques de código, blockquotes, listas, separadores, tablas
 * y párrafos con detección de patrones de documentación (Type, Returns).
 * @param {object} doc - Instancia de jsPDF.
 * @param {string} md - Contenido markdown de la página.
 * @param {number} startY - Posición Y donde comienza el contenido.
 * @param {object} ctx - Contexto con titulo y pagina para el header.
 * @returns {number} Posición Y final tras renderizar todo el contenido.
 */
function rmd(doc, md, startY, ctx) {
  ...
  while (i < lines.length) {
    ...
    // H2 con línea parcial en azul de marca
    if (/^##\s/.test(tri)) {
      y = chy(doc, y, 10, ctx);
    ...
    // Bloque de código con fondo oscuro de marca
    if (tri.startsWith('```')) {
      var clines = [];
    ...
    // Lista de viñetas con guión en accent azul
    if (/^[-*+]\s/.test(tri)) {
      var txt = lmd(tri);
    ...
    // Lista numerada con número en azul de marca
    if (/^\d+\.\s/.test(tri)) {
    ...
    // Tabla: omite líneas de separador guiones
    if (tri.startsWith('|')) {
    ...
    // Párrafo normal con detección de patrones de documentación
    var txt = lmd(tri);
    if (!txt) continue;

    // Etiqueta TYPE en azul muted con valor en courier
    if (/^type:/i.test(txt)) {
      y = chy(doc, y, 7, ctx);
    ...
    // Etiqueta RETURNS en azul muted con valor en courier
 ...
 * El script bash genera archivos UTF-8, pero se mantiene la detección de BOM
 * por si algún archivo fue editado en Windows y quedó con UTF-16.
 * @async
...
async function fetchMd(url) {
  ...
  // Por defecto UTF-8 (el script bash genera UTF-8 nativo)
  return new TextDecoder('utf-8').decode(buf);
}

/**
 * Genera y descarga un PDF con toda la documentación ordenada según el sidebar.
 * Parsea _sidebar.md para obtener el orden, descarga cada .md con fetchMd()
 * y usa jsPDF directamente (sin html2canvas) para máxima compatibilidad.
 * @async
...
async function exportarPDF() {
  ...
  try {
    // Leer el sidebar con detección de encoding para obtener el orden exacto
    ...
    doc.text('Movent', PW2 / 2, 120, { align: 'center' });
    ...
    doc.text('Vue 3 + Vite  |  UNIS 2026', PW2 / 2, 148, { align: 'center' });
    ...
    // ── Contenido: una página por cada .md ────────────────────────────────────
    var secPrev = null;
    for (var pi = 0; pi < validos.length; pi++) {
      ...
      // Banda de sección cuando cambia el grupo del sidebar
      ...
      // Barra de título de la página en azul de marca
