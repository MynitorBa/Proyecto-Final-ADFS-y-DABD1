<script>
  ...
  // Modal nueva habitación (gestión)
  let showModalNuevaHab = false;
  let nuevaHabGestion = { tipoHabitacionId: 1, camaId: 1, precioPorPersona: 0, precioPorNoche: 0, capacidadMaxima: 2, metrosCuadrados: 25, descripcion: '', estadoId: 1, cantidad: 1 };
  ...
  function abrirEliminarHotel(h) {
    hotelEliminando = h;
    showModalEliminarHotel = true;
  }

  async function confirmarEliminarHotel() {
    if (!hotelEliminando) return;
    eliminandoHotel = true;
    showModalEliminarHotel = false;
    eliminandoMasivo = true;
    eliminandoMasivoProgreso = `Eliminando hotel "${hotelEliminando.nombre}" y todos sus datos...`;
    try {
  ...
  // Eliminar hotel desde vista detalle
  function abrirEliminarHotelDetalle() {
    hotelEliminando = hotelDetalle;
    showModalEliminarHotel = true;
  }

  async function confirmarEliminarHotelDetalle() {
    if (!hotelEliminando) return;
    eliminandoHotel = true;
    showModalEliminarHotel = false;
    eliminandoMasivo = true;
  ...
  function pedirEliminarImgHotel(imagenId) {
    pedirConfirmacion('Eliminar imagen', '¿Eliminar esta imagen del hotel?', () => _eliminarImgHotel(imagenId));
  }
  ...
  function pedirEliminarAmenidad(haId, nombre) {
    pedirConfirmacion('Eliminar amenidad', `¿Eliminar "${nombre}" y sus imágenes?`, () => _eliminarAmenidad(haId));
  }
  ...
  async function subirImagenAmenidad(event, haId) {
    ...
    try {
      ...
      subiendoImgAmenidadSet = new Set([...subiendoImgAmenidadSet].filter(id => id !== haId));
      event.target.value = '';
    }
  }

  function pedirEliminarImgAmenidad(haId, imgId) {
    pedirConfirmacion('Eliminar imagen', '¿Eliminar esta imagen de la amenidad?', () => _eliminarImgAmenidad(haId, imgId));
  }
  async function _eliminarImgAmenidad(haId, imgId) {
    try {
      ...
      if (!res.ok) throw new Error(`Error ${res.status}`);
      amenidadesHotel = amenidadesHotel.map(a => {
        if (a.id !== haId) return a;
        return { ...a, imagenesIds: (a.imagenesIds ?? []).filter(i => i !== imgId) };
      });
  ...
  function abrirEditarHabitacion(h) {
    ...
    editHabitacion = {
      tipoHabitacionId: h.tipoHabitacionId, camaId: h.camaId,
      precioPorPersona: h.precioPorPersona, precioPorNoche: h.precioPorNoche,
      capacidadMaxima: h.capacidadMaxima, metrosCuadrados: h.metrosCuadrados,
      descripcion: h.descripcion ?? '', estadoId: h.estadoId
    };
  ...
  function pedirEliminarImgHab(imagenId) {
    pedirConfirmacion('Eliminar imagen', '¿Eliminar esta imagen de la habitación?', () => _eliminarImgHab(imagenId));
  }
  ...
  function abrirEliminarHab(h) {
    habEliminando = h;
    showModalEliminarHab = true;
  }

  async function confirmarEliminarHab() {
    if (!habEliminando) return;
    eliminandoHab = true;
    showModalEliminarHab = false;
    eliminandoMasivo = true;
    eliminandoMasivoProgreso = `Eliminando habitación ${habEliminando.tipoHabitacion} #${habEliminando.id}...`;
    try {
  ...
  function abrirModalNuevaHab() {
    nuevaHabGestion = { tipoHabitacionId: 1, camaId: 1, precioPorPersona: 0, precioPorNoche: 0, capacidadMaxima: 2, metrosCuadrados: 25, descripcion: '', estadoId: 1, cantidad: 1 };
    mensajeNuevaHab = null;
    showModalNuevaHab = true;
  }
  ...
  async function crearHabGestion() {
    ...
    try {
      ...
      const camaNom = tiposCama.find(c => c.id === payload.camaId)?.nombre ?? '';
      ...
      for (let i = 0; i < cant; i++) {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.mensaje || `Error ${res.status}`);
        habitaciones = [...habitaciones, { ...payload, id: data.id, tipoHabitacion: tipoNom, tipoCama: camaNom, estado: estNom, imagenesIds: [] }];
        creadas++;
      }
      mensajeNuevaHab = { tipo: 'ok', texto: `${creadas} habitación(es) ${tipoNom} creada(s) correctamente.` };
    } catch (e) {
...
{#if eliminandoMasivo}
  ...
  <div class="adm__confirm" style="z-index:3501;text-align:center">
    ...
    <div class="adm__confirm__body" style="padding:1rem 1.5rem 1.5rem">
      ...
      <p style="margin:0;font-size:.78rem;color:#8b949e">Por favor espera, esto puede tardar unos momentos. No cierres esta página.</p>
    </div>
  </div>
{/if}
              ...
              <tr>
                <td>
                  <div class="adm__hotel-mini">
                    <div class="adm__hotel-mini-thumb">
                      {#if h.imagenesIds?.length > 0}<img src="{API_BASE}/imagenes/hotel/{h.imagenesIds[0]}" alt={h.nombre} />{:else}<span>🏨</span>{/if}
                    </div>
                    <div><p class="adm__hotel-mini-name">{h.nombre}</p><p class="adm__hotel-mini-id">ID #{h.id}</p></div>
                  </div>
                </td>
                ...
                <td>
                  <div style="display:flex;gap:.3rem">
                    ...
                    <button class="adm__icon-btn adm__icon-btn--delete" on:click={() => abrirEliminarHotel(h)} title="Eliminar hotel">
...
{:else}
  ...
  <div class="adm__detalle-header">
    ...
    <button class="adm__btn adm__btn--danger" on:click={abrirEliminarHotelDetalle} style="margin-left:auto">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
      Eliminar Hotel
    </button>
  </div>
    ...
    <div class="adm__card adm__detalle-form-card">
      ...
      <div style="display:flex; justify-content:flex-end; margin-top:1.25rem;">
        <button class="adm__btn adm__btn--primary" on:click={guardarInfoHotel} disabled={guardandoInfo}>
          {#if guardandoInfo}<svg class="adm__spinner adm__spinner--sm" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Guardando...{:else}Guardar cambios{/if}
        </button>
      </div>
    </div>
    ...
    <div class="adm__card">
      <div class="adm__img-section-header">
        ...
        <label class="adm__btn adm__btn--primary adm__upload-btn">
          {#if subiendoImgHotel}<svg class="adm__spinner adm__spinner--sm" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Subiendo...{:else}+ Agregar imagen{/if}
        ...
        <div class="adm__img-grid">
          {#each hotelDetalle.imagenesIds as imgId (imgId)}
            <div class="adm__img-card">
              ...
              <button class="adm__img-delete" on:click={() => pedirEliminarImgHotel(imgId)} title="Eliminar imagen">
      ...
      {:else}
        <div class="adm__img-empty"><p>Sin imágenes. Agrega la primera.</p></div>
      {/if}
    </div>
  ...
  {:else if tabDetalle === 'amenidades'}
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
      ...
      <button class="adm__btn adm__btn--primary" on:click={() => { showFormAmenidad = true; mensajeAmenidad = null; }}>+ Agregar Amenidad</button>
    </div>
    ...
    {#if showFormAmenidad}
      <div class="adm__wizard-subcard" style="margin-bottom:1rem">
        ...
        <div style="display:flex; gap:.75rem; justify-content:flex-end; margin-top:.75rem">
          ...
          <button class="adm__btn adm__btn--primary" on:click={agregarAmenidadHotel}>Agregar</button>
        </div>
      </div>
    {/if}
    ...
    {:else if amenidadesHotel.length === 0 && !showFormAmenidad}
      <div class="adm__img-empty" style="padding:2.5rem 0"><p>Este hotel no tiene amenidades. Agrega la primera.</p></div>
    {:else}
          ...
          <div class="adm__amenidad-card">
            <div class="adm__amenidad-header">
              ...
              <div style="display:flex; gap:.4rem; flex-shrink:0">
                ...
                {/if}
                <button class="adm__icon-btn adm__icon-btn--delete" title="Eliminar" on:click={() => pedirEliminarAmenidad(ha.id, ha.amenidadNombre)}>
              ...
              <div class="adm__img-grid adm__img-grid--sm">
                {#each (ha.imagenesIds ?? []) as imgId (imgId)}
                  <div class="adm__img-card">
                    ...
                    <button class="adm__img-delete" on:click={() => pedirEliminarImgAmenidad(ha.id, imgId)}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                  </div>
                {/each}
                <label class="adm__wizard-add-img-btn adm__upload-btn" title="Subir imagen">
                  {#if subiendoImgAmenidadSet.has(ha.id)}
                  ...
                  {/if}
                  <input type="file" accept="image/*" on:change={(e) => subirImagenAmenidad(e, ha.id)} disabled={subiendoImgAmenidadSet.has(ha.id)} style="display:none" />
                </label>
              </div>
            </div>
          </div>
  ...
  {:else if tabDetalle === 'habitaciones'}
    ...
    {:else}
      ...
      <div class="adm__card adm__card--no-pad">
        <div class="adm__table-wrap">
          <table class="adm__table">
            <thead><tr><th>Tipo</th><th>Cama</th><th>$/Noche</th><th>$/Persona</th><th>Cap.</th><th>m²</th><th>Estado</th><th>Imgs</th><th>Acciones</th></tr></thead>
            <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <div style="display:flex;gap:.3rem">
                      ...
                      <button class="adm__icon-btn adm__icon-btn--delete" on:click={() => abrirEliminarHab(h)} title="Eliminar">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
                      </button>
                    </div>
                  </td>
              ...
              {/each}
              {#if habitaciones.length === 0}<tr><td colspan="9" class="adm__empty-cell">No hay habitaciones registradas.</td></tr>{/if}
            </tbody>
          </table>
        </div>
      </div>
  ...
  <div class="adm__hotel-modal adm__hotel-modal--wide">
    <div class="adm__hotel-modal__header">
      <div class="adm__hotel-modal__thumb" style="background:#252b3b;font-size:1.4rem;display:flex;align-items:center;justify-content:center;">
        {#if habitacionEditando.imagenesIds?.length > 0}<img src="{API_BASE}/imagenes/habitacion/{habitacionEditando.imagenesIds[0]}" alt="hab" />{:else}<span>🛏</span>{/if}
      </div>
      <div class="adm__hotel-modal__info">
        ...
        <p class="adm__hotel-modal__loc">{habitacionEditando.tipoCama} · {habitacionEditando.capacidadMaxima} pers. · {habitacionEditando.metrosCuadrados} m²</p>
      </div>
      <button class="adm__rol-modal__close" on:click={cerrarModales} aria-label="Cerrar"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    <div class="adm__hotel-modal__body">
      <p class="adm__modal-section-title">Datos de la habitación</p>
      <div class="adm__form-grid">
        <div class="adm__field"><label>Tipo</label><select bind:value={editHabitacion.tipoHabitacionId}>{#each tiposHabitacion as t}<option value={t.id}>{t.nombre}</option>{/each}</select></div>
        <div class="adm__field"><label>Cama</label><select bind:value={editHabitacion.camaId}>{#each tiposCama as c}<option value={c.id}>{c.nombre}</option>{/each}</select></div>
        <div class="adm__field"><label>Precio/Noche ($)</label><input type="number" bind:value={editHabitacion.precioPorNoche} min="0" step="0.01" /></div>
        <div class="adm__field"><label>Precio/Persona ($)</label><input type="number" bind:value={editHabitacion.precioPorPersona} min="0" step="0.01" /></div>
        <div class="adm__field"><label>Capacidad máx.</label><input type="number" bind:value={editHabitacion.capacidadMaxima} min="1" /></div>
        <div class="adm__field"><label>m²</label><input type="number" bind:value={editHabitacion.metrosCuadrados} min="0" step="0.1" /></div>
      ...
      <div style="display:flex; justify-content:flex-end; margin-bottom:1.5rem;">
        <button class="adm__btn adm__btn--primary" on:click={guardarHabitacion} disabled={guardandoHabitacion}>
          {#if guardandoHabitacion}Guardando...{:else}Guardar cambios{/if}
        </button>
      </div>

      <div class="adm__modal-section-divider"></div>
      <div class="adm__img-section-header" style="margin-top:1rem">
        <p class="adm__modal-section-title" style="margin:0">Imágenes de la habitación</p>
        <label class="adm__btn adm__btn--ghost adm__upload-btn">
          {#if subiendoImgHab}Subiendo...{:else}+ Agregar imagen{/if}
        ...
        <div class="adm__img-grid adm__img-grid--sm" style="margin-top:.75rem">
          {#each habitacionEditando.imagenesIds as imgId (imgId)}
            <div class="adm__img-card">
              ...
              <button class="adm__img-delete" on:click={() => pedirEliminarImgHab(imgId)} title="Eliminar">
      ...
      {:else}
        <div class="adm__img-empty" style="padding:1.5rem 0"><p>Sin imágenes.</p></div>
      {/if}
    </div>
...
<!-- Modal NUEVA habitación (con cantidad) -->
{#if showModalNuevaHab && hotelDetalle}
  ...
  <div class="adm__hotel-modal adm__hotel-modal--wide">
    ...
    <div class="adm__hotel-modal__body">
      <p class="adm__modal-section-title">Datos de la habitación</p>
      <div class="adm__form-grid">
        <div class="adm__field"><label>Tipo</label><select bind:value={nuevaHabGestion.tipoHabitacionId}>{#each tiposHabitacion as t}<option value={t.id}>{t.nombre}</option>{/each}</select></div>
        <div class="adm__field"><label>Cama</label><select bind:value={nuevaHabGestion.camaId}>{#each tiposCama as c}<option value={c.id}>{c.nombre}</option>{/each}</select></div>
        <div class="adm__field"><label>Precio/Noche ($)</label><input type="number" bind:value={nuevaHabGestion.precioPorNoche} min="0" step="0.01" /></div>
        <div class="adm__field"><label>Precio/Persona ($)</label><input type="number" bind:value={nuevaHabGestion.precioPorPersona} min="0" step="0.01" /></div>
        <div class="adm__field"><label>Capacidad máx.</label><input type="number" bind:value={nuevaHabGestion.capacidadMaxima} min="1" /></div>
        <div class="adm__field"><label>m²</label><input type="number" bind:value={nuevaHabGestion.metrosCuadrados} min="0" step="0.1" /></div>
        ...
        <div class="adm__field">
          ...
          {#if nuevaHabGestion.cantidad > 50}<span class="adm__field-error">Máximo 50 habitaciones por lote</span>{:else if nuevaHabGestion.cantidad > 1}<span style="font-size:.72rem;color:var(--adm-blue);margin-top:2px">Se crearán {nuevaHabGestion.cantidad} habitaciones idénticas</span>{/if}
          <span style="font-size:.68rem;color:var(--adm-text-muted);margin-top:2px">Máximo 50 por lote</span>
        </div>
    ...
    <div class="adm__hotel-modal__footer">
      ...
      <button class="adm__btn adm__btn--primary" on:click={crearHabGestion} disabled={guardandoNuevaHab || nuevaHabGestion.cantidad > 50}>
        {#if guardandoNuevaHab}Creando...{:else}Crear {nuevaHabGestion.cantidad > 1 ? `${Math.min(nuevaHabGestion.cantidad, 50)} Habitaciones` : 'Habitación'}{/if}
      </button>
    </div>
  </div>
{/if}
  ...
  <div class="adm__rol-modal" style="max-width:420px">
    <div class="adm__cancel-modal__header">
      <div class="adm__cancel-modal__icon">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
      </div>
      <div>
        <p class="adm__cancel-modal__title">Eliminar Habitación</p>
        <p class="adm__cancel-modal__subtitle">{habEliminando.tipoHabitacion} — ID #{habEliminando.id}</p>
      </div>
      <button class="adm__cancel-modal__close" on:click={cerrarModales} aria-label="Cerrar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    <div class="adm__cancel-modal__body">
      <div class="adm__cancel-info-box">
        <div class="adm__cancel-info-row"><span class="adm__cancel-info-row__label">Tipo</span><span class="adm__cancel-info-row__value">{habEliminando.tipoHabitacion}</span></div>
        <div class="adm__cancel-info-row"><span class="adm__cancel-info-row__label">Cama</span><span class="adm__cancel-info-row__value">{habEliminando.tipoCama}</span></div>
        <div class="adm__cancel-info-row"><span class="adm__cancel-info-row__label">Precio/Noche</span><span class="adm__cancel-info-row__value adm__cancel-info-row__value--money">$ {habEliminando.precioPorNoche?.toFixed(2)}</span></div>
      </div>
      <div class="adm__cancel-warning">
        ...
        <span>Esta acción no se puede deshacer. Se eliminarán también las imágenes asociadas.</span>
      </div>
    </div>
    <div class="adm__cancel-modal__footer">
      <button class="adm__btn adm__btn--ghost" on:click={cerrarModales} disabled={eliminandoHab}>Cancelar</button>
      <button class="adm__btn--cancel-confirm" on:click={confirmarEliminarHab} disabled={eliminandoHab}>
        {#if eliminandoHab}Eliminando...{:else}Sí, eliminar habitación{/if}
      </button>
    </div>
  </div>
{/if}
...
{#if showModalEliminarHotel && hotelEliminando}
  ...
  <div class="adm__rol-modal" style="max-width:460px">
    <div class="adm__cancel-modal__header">
      ...
      <div>
        <p class="adm__cancel-modal__title">Eliminar Hotel</p>
        <p class="adm__cancel-modal__subtitle">{hotelEliminando.nombre} — ID #{hotelEliminando.id}</p>
      </div>
      <button class="adm__cancel-modal__close" on:click={cerrarModales} aria-label="Cerrar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    <div class="adm__cancel-modal__body">
      <div class="adm__cancel-info-box">
        ...
        <div class="adm__cancel-info-row"><span class="adm__cancel-info-row__label">Estado</span><span class="adm__cancel-info-row__value">{hotelEliminando.estado}</span></div>
      </div>
      <div class="adm__cancel-warning">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;margin-top:.1rem"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        <span>Esta acción no se puede deshacer. Se eliminarán <strong>todas</strong> las habitaciones, amenidades e imágenes asociadas al hotel.</span>
      </div>
    </div>
    <div class="adm__cancel-modal__footer">
      <button class="adm__btn adm__btn--ghost" on:click={cerrarModales} disabled={eliminandoHotel}>Cancelar</button>
      <button class="adm__btn--cancel-confirm" on:click={vistaHoteles === 'detalle' ? confirmarEliminarHotelDetalle : confirmarEliminarHotel} disabled={eliminandoHotel}>
        {#if eliminandoHotel}Eliminando...{:else}Sí, eliminar hotel{/if}
      </button>
    </div>
  </div>
{/if}
