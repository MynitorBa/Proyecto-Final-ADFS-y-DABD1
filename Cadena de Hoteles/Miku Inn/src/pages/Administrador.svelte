<script>
  ...
  // ════════════════════════════════════════════════════
  //  REPORTES — Módulo de Analíticos y Búsquedas
  // ════════════════════════════════════════════════════
  let reporteTab = 'listado'; // 'listado' | 'dashboard'

  // ── Estado listado de búsquedas ───────────────────────────────────────────
  let busquedasLog       = [];
  let cargandoBusquedas  = false;
  let errorBusquedas     = null;
  let busquedasTotal     = 0;
  let busquedasPagina    = 1;
  const BUSQUEDAS_POR_PAG = 25;

  // ── Filtros ───────────────────────────────────────────────────────────────
  let filtroDestino        = '';
  let filtroUsuarioAgencia = '';
  let filtroTipo           = 'todos'; // 'todos' | 'web' | 'rest'
  let filtroFechaDesde     = '';
  let filtroFechaHasta     = '';
  // filtros aplicados (los que se mandaron a la API)
  let filtrosActivos = { destino: '', usuarioAgencia: '', tipo: 'todos', fechaDesde: '', fechaHasta: '' };
  ...
  // ── Carga desde API ───────────────────────────────────────────────────────
  async function cargarBusquedas(pagina = 1, filtros = filtrosActivos) {
    cargandoBusquedas = true;
    errorBusquedas    = null;
    try {
      const p = new URLSearchParams({ pagina, porPagina: BUSQUEDAS_POR_PAG });
      if (filtros.destino)        p.set('destino',        filtros.destino);
      if (filtros.usuarioAgencia) p.set('usuarioAgencia', filtros.usuarioAgencia);
      if (filtros.tipo && filtros.tipo !== 'todos') p.set('tipo', filtros.tipo);
      if (filtros.fechaDesde)     p.set('fechaDesde',     filtros.fechaDesde);
      if (filtros.fechaHasta)     p.set('fechaHasta',     filtros.fechaHasta);

      const res = await fetch(`${API_BASE}/admin/reportes/busquedas?${p}`, { credentials: 'include' });
      if (!res.ok) throw new Error(`Error ${res.status}`);
      const data = await res.json();
      // Esperamos { busquedas: [...], total: N }
      busquedasLog   = data.busquedas ?? data ?? [];
      busquedasTotal = data.total ?? busquedasLog.length;
      busquedasPagina = pagina;
    } catch (e) {
  ...
  function aplicarFiltrosBusquedas() {
    filtrosActivos = {
      destino:        filtroDestino.trim(),
      usuarioAgencia: filtroUsuarioAgencia.trim(),
      tipo:           filtroTipo,
      fechaDesde:     filtroFechaDesde,
      fechaHasta:     filtroFechaHasta,
    };
    cargarBusquedas(1, filtrosActivos);
  }

  function limpiarFiltrosBusquedas() {
    filtroDestino        = '';
    filtroUsuarioAgencia = '';
    filtroTipo           = 'todos';
    filtroFechaDesde     = '';
    filtroFechaHasta     = '';
    filtrosActivos       = { destino: '', usuarioAgencia: '', tipo: 'todos', fechaDesde: '', fechaHasta: '' };
    cargarBusquedas(1, filtrosActivos);
  }

  $: busquedasTotalPaginas = Math.max(1, Math.ceil(busquedasTotal / BUSQUEDAS_POR_PAG));
  $: hayFiltrosActivos = filtrosActivos.destino || filtrosActivos.usuarioAgencia ||
                         filtrosActivos.tipo !== 'todos' || filtrosActivos.fechaDesde || filtrosActivos.fechaHasta;

  // ── Exportar ──────────────────────────────────────────────────────────────
  function abrirExportar() {
    emailExportar   = '';
    mensajeExportar = null;
    showModalExportar = true;
  }
  ...
  async function confirmarExportar() {
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/reportes/busquedas/exportar`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: emailExportar.trim(), filtros: filtrosActivos }),
      });
      if (!res.ok) throw new Error(`Error ${res.status}`);
  ...
  // ── Carga datos del dashboard de búsquedas ────────────────────────────────
  ...
  // Derived reactive: gráfica de barras de búsquedas por día (usa resumen de API)
  $: busquedasPorDia = dashBusquedasResumen?.porDia ?? [];
  $: busquedasPorTipo = {
    web:  dashBusquedasResumen?.totalWeb  ?? 0,
    rest: dashBusquedasResumen?.totalRest ?? 0,
  };
  $: topDestinos = dashBusquedasResumen?.topDestinos ?? [];
  $: maxBusquedasDia = busquedasPorDia.length
    ? Math.max(...busquedasPorDia.map(d => d.total), 1)
    : 1;
  $: maxTopDestino = topDestinos.length
    ? Math.max(...topDestinos.map(d => d.total), 1)
    : 1;
  ...
  // Función para cuando se cambia a sección reportes
  function entrarReportes() {
  ...
  <aside class="adm__sidebar">
    ...
    <nav class="adm__sidebar-nav">
      {#each navItems as item}
        <button class="adm__nav-btn" class:adm__nav-btn--active={activeSection === item.id} on:click={() => { if (item.id === 'reportes') { entrarReportes(); } else { activeSection = item.id; if (item.id === 'crear-hotel') cargarPaisesWizard(); } }}>
    ...
    <div class="adm__content">
      ...
      {:else if activeSection === 'reportes'}
        ...
        <div class="adm__rep-tabs">
          <button
            class="adm__rep-tab"
            class:adm__rep-tab--active={reporteTab === 'listado'}
            on:click={() => reporteTab = 'listado'}
          >
          ...
          <button
            class="adm__rep-tab"
            class:adm__rep-tab--active={reporteTab === 'dashboard'}
            on:click={() => reporteTab = 'dashboard'}
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        {#if reporteTab === 'listado'}
          ...
          <div class="adm__rep-filters">
            <div class="adm__rep-filters-title">
              ...
              {#if hayFiltrosActivos}
                <span class="adm__rep-filters-badge">Activos</span>
              {/if}
            </div>
            <div class="adm__rep-filters-grid">
              <div class="adm__rep-filter-field">
                <label for="f-destino">Destino / Ciudad</label>
                <input id="f-destino" type="text" bind:value={filtroDestino} placeholder="Ej: Ciudad de Guatemala" />
              </div>
              <div class="adm__rep-filter-field">
            ...
            <div class="adm__rep-filters-actions">
              <button class="adm__btn adm__btn--primary" on:click={aplicarFiltrosBusquedas}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                Buscar
              </button>
              {#if hayFiltrosActivos}
                <button class="adm__btn adm__btn--ghost" on:click={limpiarFiltrosBusquedas}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  Limpiar filtros
                </button>
              {/if}
              <div class="adm__rep-filters-spacer"></div>
              <button class="adm__btn adm__rep-export-btn" on:click={abrirExportar} disabled={busquedasTotal === 0}>
          ...
          <div class="adm__rep-results-info">
            ...
            {:else}
              <span class="adm__rep-results-label">
                {busquedasTotal.toLocaleString('es-GT')} resultado{busquedasTotal !== 1 ? 's' : ''} encontrado{busquedasTotal !== 1 ? 's' : ''}
                · Página {busquedasPagina} de {busquedasTotalPaginas}
              </span>
            {/if}
          </div>
            ...
            <div class="adm__table-wrap">
              <table class="adm__table adm__rep-table">
                ...
                <tbody>
                  {#if cargandoBusquedas}
                    <tr>
                      <td colspan="8" class="adm__empty-cell">
                        <svg class="adm__spinner" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                        Cargando búsquedas...
                      </td>
                    </tr>
                  {:else if busquedasLog.length === 0}
                    <tr>
                      <td colspan="8" class="adm__empty-cell">
                        {hayFiltrosActivos ? 'Sin resultados para los filtros aplicados.' : 'No hay búsquedas registradas aún.'}
                      </td>
                    </tr>
                  {:else}
                    {#each busquedasLog as b}
                      <tr>
                        ...
                        <td class="adm__table-mono" style="font-size:.8rem; color: var(--adm-text-muted)">
                          {b.fechaHora
                            ? new Date(b.fechaHora).toLocaleString('es-GT', { dateStyle:'short', timeStyle:'short' })
                            : '—'}
                        </td>
                      </tr>
                    {/each}
          ...
          <!-- Paginación -->
          {#if busquedasTotalPaginas > 1}
            <div class="adm__rep-pag">
              <button
                class="adm__rep-pag-btn"
                disabled={busquedasPagina <= 1 || cargandoBusquedas}
                on:click={() => cargarBusquedas(busquedasPagina - 1, filtrosActivos)}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>
                Anterior
              </button>
              <div class="adm__rep-pag-pages">
                {#each Array.from({ length: Math.min(busquedasTotalPaginas, 7) }, (_, i) => {
                    const half = 3;
                    let start = Math.max(1, busquedasPagina - half);
                    let end = Math.min(busquedasTotalPaginas, start + 6);
                    start = Math.max(1, end - 6);
                    return start + i;
                  }) as pg}
                  <button
                    class="adm__rep-pag-num"
                    class:adm__rep-pag-num--active={pg === busquedasPagina}
                    disabled={cargandoBusquedas}
                    on:click={() => cargarBusquedas(pg, filtrosActivos)}
                  >{pg}</button>
                {/each}
              </div>
              <button
                class="adm__rep-pag-btn"
                disabled={busquedasPagina >= busquedasTotalPaginas || cargandoBusquedas}
                on:click={() => cargarBusquedas(busquedasPagina + 1, filtrosActivos)}
              >
                Siguiente
        ...
        <!-- ══ TAB: DASHBOARD / GRÁFICOS ══════════════════════════════════════ -->
        {:else}
          <div class="adm__reportes-grid">

            <!-- ── GRÁFICA 1: Búsquedas Web vs REST (donut) ─────────────────── -->
              ...
              {:else}
                <div class="adm__donut-wrap">
                  <div
                    class="adm__donut"
                    role="img"
                    aria-label="Gráfica búsquedas web vs REST"
                    style="background: conic-gradient(
                      #58a6ff 0% {pctWeb}%,
                      #667eea {pctWeb}% 100%
                    );"
                  >
                    <span class="adm__donut-center" style="font-size:.85rem">
                      {(busquedasPorTipo.web + busquedasPorTipo.rest).toLocaleString('es-GT')}
                    </span>
                  </div>
                  <div class="adm__donut-legend">
                    <div class="adm__legend-item">
                      <span class="adm__legend-dot" style="background:#58a6ff"></span>
                      Portal Web ({busquedasPorTipo.web.toLocaleString('es-GT')} — {pctWeb}%)
                    </div>
                    <div class="adm__legend-item">
                      <span class="adm__legend-dot" style="background:#667eea"></span>
                      Servicio REST ({busquedasPorTipo.rest.toLocaleString('es-GT')} — {pctRest}%)
                    </div>
                    <div style="margin-top:.5rem; font-size:.78rem; color: var(--adm-text-muted)">
                      Total búsquedas registradas
                    </div>
                  </div>
                </div>
              {/if}
            ...
            <div class="adm__card adm__reporte-card">
              ...
              {#if metricas}
                ...
                {@const pConf = (confirmadas / totalRes) * 100}
                {@const pPend = (pendientes  / totalRes) * 100}
                {@const pCanc = (canceladas  / totalRes) * 100}
                {@const pExp  = (expiradas   / totalRes) * 100}
                {@const s1 = pConf}
                {@const s2 = s1 + pPend}
                {@const s3 = s2 + pCanc}
                {@const gradient = `conic-gradient(
                  #3fb950 0% ${s1.toFixed(2)}%,
                  #d29922 ${s1.toFixed(2)}% ${s2.toFixed(2)}%,
                  #f85149 ${s2.toFixed(2)}% ${s3.toFixed(2)}%,
                  #6b7280 ${s3.toFixed(2)}% 100%
                )`}
                <div class="adm__donut-wrap">
                  <div
                    class="adm__donut"
                    role="img"
                    aria-label="Gráfica de reservas por estado"
            ...
            <!-- ── GRÁFICA 3: Top 5 destinos más buscados ─────────────────── -->
            <div class="adm__card adm__reporte-card adm__reporte-card--wide">
              <div class="adm__card-header">
                <h3 class="adm__card-title">Top 5 Destinos Más Buscados</h3>
                <span style="font-size:.78rem; color: var(--adm-text-muted)">Últimos 30 días</span>
              </div>
              {#if cargandoDashBusquedas}
                <div class="adm__loading-state" style="padding:1.5rem 0">
                  <svg class="adm__spinner" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
              ...
              {:else}
                <div class="adm__hotel-bars">
                  {#each topDestinos.slice(0, 5) as dest, i}
                    <div class="adm__hotel-bar-item">
                      <div class="adm__hotel-bar-info">
                        <span class="adm__hotel-bar-name">
                          <span style="color: var(--adm-accent); font-weight:700; margin-right:.35rem">#{i+1}</span>
                          {dest.nombre ?? dest.destino ?? dest.ciudad ?? '—'}
                        </span>
                        <span class="adm__hotel-bar-pct">{dest.total.toLocaleString('es-GT')} búsquedas</span>
                      </div>
                      <div class="adm__hotel-bar-track">
                        <div
                          class="adm__hotel-bar-fill"
                          style="width:{Math.round((dest.total / maxTopDestino) * 100)}%;
            ...
            <!-- ── GRÁFICA 4: Búsquedas por día (últimos 14 días) ────────── -->
            <div class="adm__card adm__reporte-card adm__reporte-card--wide">
              <div class="adm__card-header">
                <h3 class="adm__card-title">Volumen de Búsquedas por Día</h3>
                <span style="font-size:.78rem; color: var(--adm-text-muted)">Últimos 14 días</span>
              </div>
              {#if cargandoDashBusquedas}
                <div class="adm__loading-state" style="padding:1.5rem 0">
                  <svg class="adm__spinner" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                ...
                <div class="adm__rep-bar-chart">
                  {#each busquedasPorDia.slice(-14) as dia}
                    <div class="adm__rep-bar-col">
                      ...
                      <div class="adm__rep-bar-track">
                        <div
                          class="adm__rep-bar-fill"
                          style="height:{Math.round((dia.total / maxBusquedasDia) * 100)}%"
                        ></div>
                      </div>
                      <div class="adm__rep-bar-lbl">
                        {new Date((dia.dia ?? dia.fecha) + 'T00:00:00').toLocaleDateString('es-GT', { day:'2-digit', month:'short' })}
                      </div>
                    </div>
                  {/each}
                </div>
              ...
              <div class="adm__hotel-bars">
                {#each [...hoteles].sort((a,b) => b.rating - a.rating).slice(0, 6) as h}
                  <div class="adm__hotel-bar-item">
                    <div class="adm__hotel-bar-info">
                      <span class="adm__hotel-bar-name">{h.nombre}</span>
                      <span class="adm__hotel-bar-pct">★ {h.rating?.toFixed(1)}</span>
                    </div>
                    <div class="adm__hotel-bar-track">
                      <div class="adm__hotel-bar-fill" style="width: {Math.round((h.rating / 5) * 100)}%"></div>
                    </div>
        ...
        {#if showModalExportar}
          <div
            class="adm__overlay"
            on:click={cerrarExportar}
            on:keydown={e => e.key === 'Escape' && cerrarExportar()}
            role="button" tabindex="-1" aria-label="Cerrar"
          ></div>
          <div class="adm__rep-export-modal">
            <div class="adm__rep-export-modal__header">
              <div style="display:flex;align-items:center;gap:.75rem">
                <div class="adm__rep-export-modal__icon">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                </div>
                <div>
            ...
            <div style="padding:1.5rem;background:#1e2a3a">
              <p style="font-size:.875rem;color:#b0c4d8;margin:0 0 1rem">
                Se exportarán <strong style="color:#e8f0fe">{busquedasTotal.toLocaleString('es-GT')} registros</strong>
                {#if hayFiltrosActivos}con los filtros activos{:else}(sin filtros){/if}
                y se enviarán al correo indicado.
              </p>
              <div style="display:flex;flex-direction:column;gap:.35rem;margin-bottom:1rem">
                <label for="exp-email" style="font-size:.78rem;font-weight:600;color:#7eb8d4;text-transform:uppercase;letter-spacing:.05em">Correo electrónico</label>
                <input
                  id="exp-email"
                  type="email"
                  bind:value={emailExportar}
                  placeholder="correo@ejemplo.com"
                  style="width:100%;padding:.7rem .9rem;background:#253142;border:1px solid #3a4a5c;border-radius:8px;color:#e8f0fe;font-size:.9rem;font-family:inherit;box-sizing:border-box;outline:none"
                  on:focus={e=>{e.target.style.borderColor='#667eea';e.target.style.boxShadow='0 0 0 3px rgba(102,126,234,.15)'}}
                  on:blur={e=>{e.target.style.borderColor='#3a4a5c';e.target.style.boxShadow='none'}}
                  on:keydown={e => e.key === 'Enter' && confirmarExportar()}
                />
              </div>
              {#if mensajeExportar}
                <div style="padding:.65rem 1rem;border-radius:8px;font-size:.85rem;font-weight:500;
                  background:{mensajeExportar.tipo==='ok' ? 'rgba(63,185,80,.15)' : 'rgba(248,81,73,.15)'};
                  border:1px solid {mensajeExportar.tipo==='ok' ? 'rgba(63,185,80,.4)' : 'rgba(248,81,73,.4)'};
                  color:{mensajeExportar.tipo==='ok' ? '#3fb950' : '#f85149'}">
                  {mensajeExportar.texto}
                </div>
              {/if}
            </div>
            <div style="display:flex;justify-content:flex-end;gap:.75rem;padding:1rem 1.5rem;border-top:1px solid #3a4a5c;background:#253142">
              <button on:click={cerrarExportar} disabled={exportando}
                style="padding:.6rem 1.1rem;background:transparent;border:1px solid #3a4a5c;border-radius:8px;color:#b0c4d8;font-size:.875rem;font-weight:500;cursor:pointer">
                Cancelar
              </button>
              <button on:click={confirmarExportar} disabled={exportando}
                style="padding:.6rem 1.3rem;background:linear-gradient(135deg,#667eea,#764ba2);border:none;border-radius:8px;color:#fff;font-size:.875rem;font-weight:600;cursor:pointer;opacity:{exportando?'.6':'1'}">
                {#if exportando}
                  <svg class="adm__spinner adm__spinner--sm" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                  Enviando...
                {:else}
