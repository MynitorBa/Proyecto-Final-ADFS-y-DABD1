...
async function exportarPDF() {
  ...
  try {
    ...
    // Filtrar nulls y excluir Tests del PDF
    var validos = contenidos.filter(function(p) {
      if (!p) return false;
      var tituloLower  = (p.titulo  || '').toLowerCase();
      var archivoLower = (p.archivo || '').toLowerCase();
      if (tituloLower === 'tests' || tituloLower === 'test') return false;
      if (archivoLower === 'tests' || archivoLower === 'test') return false;
      return true;
    });
    ...
    doc.text('Java + Javalin  |  UNIS 2026', PW2 / 2, 148, { align: 'center' });
