...
// @ts-nocheck
  ...
  // ── Filtros ──────────────────────────────────────────────────────
  let filtroEstado  = 'todos';   // todos | Activo | Cancelado | Finalizado
  let filtroBusqueda = '';       // número de vuelo, origen, destino
  ...
  async function cargarHistorial() {
    ...
    } catch { mostrarToast('error', 'Error de conexión al cargar el historial'); }
  ...
  async function handleCancelarVuelo(vueloId) {
    ...
    } catch { mostrarToast('error', 'Error de conexión al cancelar el vuelo'); }
  }
</script>
                ...
                <td class="table__cell">
                  {#if vuelo.fechaLlegada && vuelo.fechaLlegada !== vuelo.fecha}
                    <span class="fecha-llegada-distinta">
                      {vuelo.fechaLlegada}
                      <span class="nextday-tag">+día</span>
                    </span>
                  {:else}
